...
import sys
sys.path.append("D:\\实习\\SmartSpeaker\\chatboot\\SVC")
...
def transfer(count):
    ...
    # 一定要设置的部分
    parser.add_argument('-m', '--model_path', type=str, default="SVC/logs/44k/hutao_G_40000.pth", help='模型路径')
    parser.add_argument('-c', '--config_path', type=str, default="SVC/configs/hutao.json", help='配置文件路径')
    parser.add_argument('-cl', '--clip', type=float, default=0, help='音频强制切片，默认0为自动切片，单位为秒/s')
    # parser.add_argument('-n', '--clean_names', type=str, nargs='+', default=["君の知らない物語-src.wav"], help='wav文件名列表，放在raw文件夹下')
    parser.add_argument('-n', '--clean_names', type=str, nargs='+', default=["speech.wav"], help='wav文件名列表，放在raw文件夹下')
    parser.add_argument('-t', '--trans', type=int, nargs='+', default=[6], help='音高调整，支持正负（半音）')
    parser.add_argument('-s', '--spk_list', type=str, nargs='+', default=['hutao'], help='合成目标说话人名称')
    ...
    # 浅扩散设置
    parser.add_argument('-dm', '--diffusion_model_path', type=str, default="SVC/logs/44k/diffusion/model_0.pt", help='扩散模型路径')
    parser.add_argument('-dc', '--diffusion_config_path', type=str, default="SVC/logs/44k/diffusion/config.yaml", help='扩散模型配置文件路径')
    ...
    if cluster_infer_ratio != 0:
        if args.cluster_model_path == "":
            if args.feature_retrieval:  # 若指定了占比但没有指定模型路径，则按是否使用特征检索分配默认的模型路径
                args.cluster_model_path = "SVC/logs/44k/feature_and_index.pkl"
            else:
                args.cluster_model_path = "SVC/logs/44k/kmeans_10000.pt"
    ...
    for clean_name, tran in zip(clean_names, trans):
        raw_audio_path = f"SVC/raw/{clean_name}"
        ...
        for spk in spk_list:
            ...
            res_path = f'SVC/results/{clean_name}_{key}_{spk}{cluster_name}_{isdiffusion}_{f0p}_{count}.{wav_format}'
            # res_path = f'SVC/results/after.{wav_format}'
